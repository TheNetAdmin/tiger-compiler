...
void PrintIRTree::printStm(std::shared_ptr<IR::Stm> stm, std::ostream &outFile, int i)
{
    switch (stm->getStmType())
    {
        case IR::SEQ:
        {
            ...
            printBlank(i, outFile);outFile << "      ";printStm(seq->getLeft(), outFile, i + 5);
            printBlank(i, outFile);outFile << "      ";printStm(seq->getRight(), outFile, i + 5);
            outFile << std::endl;
            break;
        }
        ...
        case IR::LABEL:
        {
            ...
            printBlank(i, outFile);outFile << "       | — " << label->getLabel()->getLabelName();
            outFile << std::endl;
            break;
        }
        ...
        case IR::JUMP:
        {
            ...
            printBlank(i, outFile);outFile << "      "; printExp(jump->getExp(), outFile, i + 6);
            outFile << std::endl;
            break;
        }
        ...
        case IR::CJUMP:
        {
            ...
            printBlank(i, outFile);outFile << "       | — " << rel_oper[cjump->getOp()] << std::endl;
            printBlank(i, outFile);outFile << "       ";printStm(cjump->getLeft(), outFile, i + 7);
            printBlank(i, outFile);outFile << "       ";printStm(cjump->getRight(), outFile, i + 7);
            printBlank(i, outFile);outFile << "       | — " << (cjump->getLabelTrue() ? cjump->getLabelTrue()->getLabelName(): "NULL");outFile << std::endl;
            printBlank(i, outFile);outFile << "       | — " << (cjump->getLabelTrue() ? cjump->getLabelFalse()->getLabelName(): "NULL");
            outFile << std::endl;
            break;
        }
        ...
        case IR::MOVE:
        {
            ...
            printBlank(i, outFile);outFile << "      ";printStm(move->getDst(), outFile, i + 5);
            printBlank(i, outFile);outFile << "      ";printStm(move->getSrc(), outFile, i + 5);
            outFile << std::endl;
            break;
        }
        ...
        case IR::EXP:
        {
            ...
            printBlank(i, outFile);outFile << "      "; printExp(exp, outFile, i + 6);
            outFile << std::endl;
            break;
        }
    }
...
void PrintIRTree::printExp(std::shared_ptr<IR::Exp> exp, std::ostream &outFile, int i)
{
    switch (exp->getExpType())
    {
        case IR::BINOP:
        {
            ...
            printBlank(i, outFile);outFile << "       | — " <<  bin_oper[binop->getOp()] << std::endl;
            printBlank(i, outFile);outFile << "       ";printStm(binop->getLeft(), outFile, i + 6);
            printBlank(i, outFile);outFile << "       ";printStm(binop->getRight(), outFile, i + 6);
            outFile << std::endl;
            break;
        }
        ...
        case IR::MEM:
        {
            ...
            printBlank(i, outFile);outFile << "      ";printExp(mem->getExp(), outFile, i + 6);
            outFile << std::endl;
            break;
        }
        ...
        case IR::ESEQ:
        {
            ...
            printBlank(i, outFile);outFile << "      ";printStm(eseq->getStm(), outFile, i + 5);
            printBlank(i, outFile);outFile << "      ";printExp(eseq->getExp(), outFile, i + 5);
            outFile << std::endl;
            break;
        }
        ...
        case IR::CALL:
        {
            ...
            printBlank(i, outFile);outFile << "      ";printExp(call->getFun(), outFile, i + 6);
...
int PrintIRTree::printStmDot(std::shared_ptr<IR::Stm> stm, std::ostream &outFile)
{
    switch (stm->getStmType())
    {
        ...
        case IR::LABEL:
        {
            ...
            outFile << "node" << childnode << "[label = \"<f0>|<f1> "<< label->getLabel()->getLabelName() << "|<f2>\"]" << std::endl;
        ...
        case IR::CJUMP:
        {
            auto cjump = std::dynamic_pointer_cast<IR::CJump>(stm);
            int mynode = ++nodeNum;
            outFile << "node" << mynode << "[label = \"<f0>" << (cjump->getLabelTrue() ? cjump->getLabelTrue()->getLabelName(): "NULL")
                    <<"|<f1> CJUMP: " << rel_oper[cjump->getOp()] << " |<f2>" << (cjump->getLabelTrue() ? cjump->getLabelFalse()->getLabelName(): "NULL")
                    << "\"]" << std::endl;
...
int PrintIRTree::printExpDot(std::shared_ptr<IR::Exp> exp, std::ostream &outFile)
{
    switch (exp->getExpType())
    {
        case IR::BINOP:
        {
            auto binop = std::dynamic_pointer_cast<IR::Binop>(exp);
            int mynode = ++nodeNum;
            outFile << "node" << mynode << "[label = \"<f0>|<f1> BINOP: " << bin_oper[binop->getOp()] << " |<f2>\"]" << std::endl;
        ...
        case IR::TEMP:
        {
            auto temp = std::dynamic_pointer_cast<IR::Temp>(exp);
            int mynode = ++nodeNum;
            outFile << "node" << mynode << "[label = \"<f0>|<f1> TEMP: " << temp->getTemp()->getTempName() <<"|<f2>\"]" << std::endl;
            return mynode;
        }
        ...
        case IR::NAME:
        {
            auto name = std::dynamic_pointer_cast<IR::Name>(exp);
            int mynode = ++nodeNum;
            outFile << "node" << mynode << "[label = \"<f0>|<f1> NAME: " << name->getLabel()->getLabelName() <<"|<f2>\"]" << std::endl;
            return mynode;
        }
        ...
        case IR::CONST:
        {
            auto constt = std::dynamic_pointer_cast<IR::Const>(exp);
            int mynode = ++nodeNum;
            outFile << "node" << mynode << "[label = \"<f0>|<f1> CONST: " << constt->getConstt() <<"|<f2>\"]" << std::endl;
            return mynode;
        }
...
void PrintIRTree::makeDotFile(std::ofstream &outFile)
{
    if (fragList == nullptr)
    {
        outFile << "FragList is empty" << std::endl;
