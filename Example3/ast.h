...
struct ifelseBlock : public Expression
{
    ...
    ifelseBlock(Expression *_cond, Expression *_thenBlock, Expression *_elseBlock) : cond(_cond), thenBlock(_thenBlock), elseBlock(_elseBlock)
    {
	if (_cond != NULL)
	    _cond->father = this;
	if (_thenBlock != NULL)
...
struct forBlock : public Expression
{
    ...
    forBlock(Ident *_id, Expression *_loopFrom, Expression *_loopTo, Expression *_loopBlock) : id(_id), loopFrom(_loopFrom), loopTo(_loopTo), loopBlock(_loopBlock)
    {
	if (_id != NULL)
	    _id->father = this;
	if (_loopFrom != NULL)
	    _loopFrom->father = this;
	if (_loopTo != NULL)
	    _loopTo->father = this;
	if (_loopBlock != NULL)
	    _loopBlock->father = this;
    }
    virtual void gen();
};
